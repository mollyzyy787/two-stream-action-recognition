...
def record_info(info,filename,mode):
    ...
    if mode =='train':
        ...
        result = (
              ...
              'LR {lr}\n'.format(batch_time=info['Batch Time'],
               data_time=info['Data Time'], loss=info['Loss'], top1=info['Prec@1'], top3=info['Prec@3'],lr=info['lr']))
        print(result)

        df = pd.DataFrame.from_dict(info)
        column_names = ['Epoch','Batch Time','Data Time','Loss','Prec@1','Prec@3','lr']
    ...
    if mode =='test':
        result = (
              ...
              'Prec@1 {top1} '
              'Prec@3 {top3} \n'.format( batch_time=info['Batch Time'],
               loss=info['Loss'], top1=info['Prec@1'], top3=info['Prec@3']))
        print(result)
        df = pd.DataFrame.from_dict(info)
        column_names = ['Epoch','Batch Time','Loss','Prec@1','Prec@3']
