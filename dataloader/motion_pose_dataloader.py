...
class motion_pose_dataset(Dataset):
    ...
    def stackPoseFlow(self):
        ...
        for j in range(self.in_channel):
            ...
            pose_image = self.root_dir + name + '/' + frame_idx +'.jpg'
            poseImg=(Image.open(pose_image))

            img = self.transform(poseImg)
...
class Motion_Pose_DataLoader():
    ...
    def train(self):
        training_set = motion_pose_dataset(dic=self.dic_video_train, in_channel=self.in_channel, root_dir=self.data_path,
            mode='train',
    ...
    def val(self):
        validation_set = motion_pose_dataset(dic= self.dic_test_idx, in_channel=self.in_channel, root_dir=self.data_path ,
            mode ='val',
            transform = transforms.Compose([
            transforms.Resize([224,224]),
            transforms.ToTensor(),
...
if __name__ == '__main__':
    data_loader =Motion_Pose_DataLoader(BATCH_SIZE=1,num_workers=1,in_channel=10,
